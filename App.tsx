...
const App: React.FC = () => {
  ...
  const fetchPredictions = useCallback(async (selectedDate: string) => {
    ...
    try {
      ...
      // Extract formatted grounding chunks for UI display
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: "Unable to retrieve real-time football data. Please check your internet connection and try again."
      }));
    }
  }, []);
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xl font-bold text-white flex items-center space-x-2">
              ...
              <span>13 Daily Selections</span>
            </h2>
            <p className="text-sm text-gray-400">
              Generated for: <span className="text-emerald-400 font-mono">{state.date}</span>
            </p>
          </div>
          <p className="text-gray-400 text-sm max-w-2xl">
            Our AI analyzes live trends, team news, and historical data from top platforms like BeSoccer and sofascore to provide these calculated probabilities.
          </p>
        </div>
      ...
      <footer className="mt-auto border-t border-white/10 py-8 bg-slate-900/80">
        <div className="container mx-auto px-4 text-center">
          <p className="text-gray-500 text-xs mb-2">
            Disclaimer: Predictions are generated by AI for informational purposes only. Responsible gambling is advised.
          </p>
          <p className="text-emerald-500/50 text-[10px] font-bold tracking-widest uppercase">
            &copy; 2024 CUSTOMBET AI - PRO POWERED PREDICTIONS
          </p>
        </div>
      </footer>
    </div>
